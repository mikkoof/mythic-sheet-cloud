...
export async function createSteedAction(knightId: string, formData: FormData) {
  const { knight } = await requireKnightWriteAccess(knightId);
  const input = createCompanionSchema.parse({
    name: formData.get("name") ?? "",
  });

  await prisma.steed.create({
    data: {
      knightId: knight.id,
      name: input.name,
    },
...
export async function updateSteedAction(steedId: string, formData: FormData) {
  const steed = await prisma.steed.findUnique({ where: { id: steedId } });
  if (!steed) notFound();
  const { knight } = await requireKnightWriteAccess(steed.knightId);
...
export async function deleteSteedAction(steedId: string) {
  const steed = await prisma.steed.findUnique({ where: { id: steedId } });
  if (!steed) notFound();
  const { knight } = await requireKnightWriteAccess(steed.knightId);

  await prisma.steed.delete({ where: { id: steedId } });
